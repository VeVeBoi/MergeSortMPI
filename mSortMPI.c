...
void mergeSortP(double *arr, int left, int right);
...
void serial(int argc, char** argv){
    ...
    //printf("%d | %s | %f\n",atoi(argv[1]), "1", endTime-startTime);
...
void parallel(int argc, char** argv, int comm_sz, int rank, MPI_Comm comm){
    ...
    //MPI_Gatherv(recBuffer, sendcount[rank], MPI_DOUBLE, sorted, sendcount, displs, MPI_DOUBLE, 0, comm);
...
void mergeSortP(double *arr, int left, int right){
    ...
    int lowCount = mid - left + 1;
    int upperCount = right - mid;
    ...
    if(right == left){
        ...
        aux = mergeP(arr+left, lowCount, arr+mid+1, upperCount);
    }
}
